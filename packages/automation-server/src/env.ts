...
export const env = createEnv({
  server: {
    ...
    // When true, /generate returns a canned response instead of driving a real
    // browser session — useful for exercising queue/db/storage wiring without
    // a running Open-Pencil frontend or AI credentials.
    USE_AI_STUB: z
      .string()
      .transform((v) => v.toLowerCase() === "true")
      .default(false),

    STORE_PROVIDER: z.enum(["fs", "s3"]).default("fs"),
    S3_ENDPOINT: z.string().min(1).optional(),
    // Required when STORE_PROVIDER is set to s3
    ...
    /** Directory for local filesystem storage. */
    ...
    // How many /generate jobs this worker processes concurrently. Each one maps
    // to one browser session in @open-pencil/automation — keep this at or below
    // that package's OPENPENCIL_MAX_CONCURRENCY, or jobs will just queue up
    // waiting for a free browser context instead of running in the queue.
